<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-surface-800 rounded-xl border border-surface-200 dark:border-surface-700 shadow-sm">
            <div class="p-6">
                <form action="{{ route('servers.import.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                        ...
                        <div class="space-y-3">
                            <label class="flex items-start p-4 border border-surface-200 dark:border-surface-600 rounded-lg cursor-pointer hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors">
                                ...
                                <div class="ml-3">
                                    ...
                                    <span class="block text-sm text-surface-500 dark:text-surface-400">{{ __('app.import_mode_skip_desc') }}</span>
                                </div>
                            </label>
                            <label class="flex items-start p-4 border border-surface-200 dark:border-surface-600 rounded-lg cursor-pointer hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors">
                                ...
                                <div class="ml-3">
                                    ...
                                    <span class="block text-sm text-surface-500 dark:text-surface-400">{{ __('app.import_mode_update_desc') }}</span>
                                </div>
                            </label>
                        </div>
                        @error('mode')
                    ...
                    <div class="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="mt-1 text-sm text-amber-700 dark:text-amber-300">{{ __('app.import_warning') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit" 
                                class="px-4 py-2.5 text-sm font-medium rounded-lg bg-primary-600 text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors">
